    ...
    <div class="col-md-6">
        <div class="card overflow-auto" style="scrollbar-width: thin">
            <table class="table table-bordered table-hover table-striped data-table text-sm text-nowrap align-middle">
                ...
                <tbody>
                    @foreach ($all_vendor_Products as $product)
                        ...
                        <tr class="text-center">
                            ...
                            @if ($product->created_at != null)
                                <td>
                                    ...
                                    <small style="color:#ff00ff">{{ $product->created_at->format('h:i A') }}</small>
                                </td>
                            @else
                            ...
                            @if ($product->updated_at == $product->created_at)
                                <td class="text-danger text-center">Data not updated yet. </td>
                            @else
                                <td>
                                    {{ $product->updated_at->format('d/m/Y') }}
                                    <i style="color:#ff00ff">{{ $product->updated_at->format('h:i A') }}</i>
                                </td>
                            @endif
                        </tr>
                    @endforeach
    ...
    <div class="col-md-12">
        <div class="table-responsive" style="scrollbar-width: thin">
            <table class="table table-bordered table-hover table-data text-sm-nowrap align-middle">
                ...
                <tbody>
                    @if ($all_vendor_Products->isNotEmpty())
                        @foreach ($all_vendor_Products as $product)
                            <tr>
                                ...
                                <td class="td-13">
                                    ...
                                    @else
                                        <div class="mx-auto d-flex align-items-center justify-content-center btn btn-outline-warning p-1 border border-1 border-danger rounded"
                                            style="width: 80px; height: 80px;">
                                            <span class="small text-center text-danger">No Image</span>
                                        </div>
                                    @endif
                                    @if ($totalFile > 0)
                                ...
                                <td class="td-6 text-center">
                                    <a href="{{ route('vendor.product.status', $product->id) }}"
            ...
            <div class="d-flex justify-content-end mx-1">
                {{ $all_vendor_Products->onEachSide(1)->links() }}
            </div>
        </div>
    </div>
</div>
